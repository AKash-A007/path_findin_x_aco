...
class ACO_PDG():
    ...
    def __init__(self, width, height, obstacles, survivor_field, ant_count, step_count, alpha, beta, gamma, evaporation_rate, start, end, brushfire_iter, colony_iter):
        self.width = width
        self.height = height
        self.obstacles = obstacles
        self.survivor_field = survivor_field  
    ...
    def U_att(self, p, p_e):
        ...
        #euclidian distance between the goa and the currecnt node make it to queadritic form
        return K_a * np.linalg.norm(p - p_e) ** 2 / 2.

    def U_rep(self, rho, rho_o):
        K_r = 25.
        #replusion to an obstacle
        if rho <= rho_o:
    ...
    def Potential_Field_Heuristic(self, brushfire_iter):
        ...
        # Survivor field influence
        for survivor in self.survivor_field:
            x, y, weight = survivor
            # Decrease heuristic for survivor field cells by weight
            heuristic_grid[x, y] *= (1 - weight)  # More attractive if weight is higher
    ...
    def move_ant(self):
        ...
        for i in range(self.step_count):
            ...
            for can_go in can_go_list:
                pheromone_influence = self.pheromone_grid[can_go[0], can_go[1]] ** self.alpha
                heuristic_influence = self.heuristic_grid[can_go[0], can_go[1]] ** self.beta
                # Survivor field boost: increase pheromone if in the survivor field
                if (can_go[0], can_go[1]) in [(x, y) for x, y, w in self.survivor_field]:
                    pheromone_influence *= 3  # Boost pheromone in survivor field cells
                p.append(pheromone_influence * heuristic_influence)
    ...
    def plot(self, path):
        ...
        for survivor in self.survivor_field:
            table[survivor[0], survivor[1]] = 0.9  # Visualizing the survivor field with a distinct color
        for grid in path:
    ...
    def __repr__(self):
        o = str(self.width) + "x" + str(self.height) + " grid with obstacles and survivor field:\n"
        o += str(self.obstacles) + "\nSurvivor field: " + str(self.survivor_field)
        return o
...
if __name__ == "__main__":
    ...
    Obstacles = [(0, 2), (0, 3), (0, 10), (0, 11), 
                 ...
                 (17, 3), (17, 6),(3,0),(3,1),(3,2)]
    
    survivor_field = [(8,16,0.9), (8, 15, 0.9), (9, 15, 0.9), (6, 6, 0.6)]    
    aco_pdg = ACO_PDG(20, 20, Obstacles, survivor_field, 20, 100, 1.1, 12, 0.02, 0.5, np.array([0, 0]), np.array([8,19 ]), 5, 20)
